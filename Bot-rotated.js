...
const config = (() => {
    const DEFAULTS = {
        ...
        cf_timeout: 60000, cf_cache: false, cf_concurrency: 3, cf_browsers: 3, cf_rotate_after: 15,
    };
    let raw = {};
...
// ─── Inline CF Scraper (puppeteer-real-browser, rotation + debug logging) ─────
const cfScraper = {
    _browsers: [],       // browser instances (null = launching)
    _solveCounts: [],    // solves per browser slot
    _rotating: [],       // true if slot is currently rotating
    _ready: false,
    _launching: false,
    ...
    async init() {
        ...
        const count = config.cf_browsers || 3;
        console.log(`  CF Scraper: launching ${count} browser(s)...`);
        this._solveCounts = new Array(count).fill(0);
        this._rotating = new Array(count).fill(false);
    ...
    async _launchOne(idx) {
        ...
        try {
            ...
            this._solveCounts[idx] = 0;
            this._rotating[idx] = false;
            console.log(`  CF Browser #${idx + 1} launched (fresh fingerprint)`);
    ...
    // Rotate: close old browser, launch fresh one (new fingerprint)
    async _rotate(idx) {
        if (this._rotating[idx]) return;
        this._rotating[idx] = true;
        const old = this._browsers[idx];
        this._browsers[idx] = null;
        console.log(`  ♻ Rotating Browser #${idx + 1} (${this._solveCounts[idx]} solves → fresh fingerprint)...`);
        if (old) {
            try { old.removeAllListeners('disconnected'); await old.close(); } catch (_) { }
        }
    ...
    _pickBrowser() {
        let best = null, bestCount = Infinity, bestIdx = -1;
        for (let i = 0; i < this._browsers.length; i++) {
            if (this._browsers[i] && !this._rotating[i] && this._solveCounts[i] < bestCount) {
                best = this._browsers[i];
                bestCount = this._solveCounts[i];
                bestIdx = i;
            }
        }
        if (bestIdx >= 0) this._solveCounts[bestIdx]++;
        const rotateAfter = config.cf_rotate_after || 15;
        if (bestIdx >= 0 && this._solveCounts[bestIdx] >= rotateAfter) {
            this._rotate(bestIdx);
        }
        return best;
    },
    ...
    async _doSolve(url, proxy) {
        ...
        return new Promise(async (resolve, reject) => {
            const timer = setTimeout(async () => {
                ...
                if (!resolved) { await context.close().catch(() => { }); reject(new Error('CF scraper: timeout')); }
            }, timeout);
            try {
                ...
                if (config.debugmode) {
                    page.on('requestfailed', req => console.warn(`  [CF requestfailed] ${req.url()} ${req.failure()?.errorText || ''}`));
                    page.on('pageerror', err => console.warn(`  [CF pageerror] ${err.message}`));
                    page.on('console', msg => console.log(`  [CF console] ${msg.type()}: ${msg.text()}`));
                    page.on('framenavigated', frame => console.log(`  [CF frame] navigated to ${frame.url()}`));
                }
                if (config.debugmode) {
                    page.on('response', res => console.log(`  [CF response] ${res.status()} ${res.url()}`));
                }
                await page.goto(url, { waitUntil: 'domcontentloaded' });
                // Debug-only screenshots
                if (config.debugmode) {
                    console.log(`  [CF] goto finished, url=${await page.url()}`);
                    const pageHtml = await page.content();
                    console.log(`  [CF] page HTML length=${pageHtml.length}`);
                    console.log(`  [CF] page HTML preview:\n${pageHtml.slice(0, 2000)}`);
                    const screenshotPath = path.join(__dirname, 'puppeteer-cf.png');
                    const captureScreenshot = async () => {
                        try {
                            await page.screenshot({ path: screenshotPath, fullPage: true });
                            console.log(`  [CF screenshot] updated ${screenshotPath}`);
                        } catch (err) {
                            console.warn(`  [CF screenshot error] ${err.message}`);
                        }
                    };
                ...
                resolve({ cookies, headers, acceptLang });
            } catch (e) {
                if (!resolved) {
                    clearTimeout(timer);
                    if (screenshotInterval) clearInterval(screenshotInterval);
...
class BotPool extends EventEmitter {
    ...
    addBot(bot) { this.bots.push(bot); this._botSet.add(bot); }
    ...
    remove(bot) { if (!this._botSet.has(bot)) return; this._botSet.delete(bot); const i = this.bots.indexOf(bot); if (i !== -1) this.bots.splice(i, 1); }
