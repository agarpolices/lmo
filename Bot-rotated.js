...
// ─── Inline CF Scraper (puppeteer-real-browser, rotation + fingerprint refresh) ─
...
const cfScraper = {
    ...
    async init() {
        ...
        // Pre-import puppeteer-real-browser once
        _puppeteerRealBrowser = await import('puppeteer-real-browser');
    ...
    async _launchOne(idx) {
        ...
        const { connect } = _puppeteerRealBrowser;
        try {
    ...
    // Concurrency limiter (linked-list queue for O(1) dequeue)
    _active: 0,
    ...
    async _doSolve(url, proxy) {
        ...
        try {
            ...
            // Debug listeners (only attach if needed)
            if (config.debugmode) {
